...
EMINENT_PRICE_Y_OFFSET_RATIO = 0.055


def price_anchor_y(marker, pill):
    if not marker.startswith("@"):
        return pill["center_y"]
    offset = min(7, max(3, pill["height"] * EMINENT_PRICE_Y_OFFSET_RATIO))
    return pill["center_y"] + offset
...
def center_and_fit_price(node, price_text, pill):
    ...
    if pill:
        ...
        node.set("y", f"{price_anchor_y(original_marker, pill):.3f}".rstrip("0").rstrip("."))
    else:
